...
class GetCastCreditsByIdUseCaseTest {
    ...
    @Test
    fun getCastCreditsById_ShouldReturnListOfCrewCreditsEntityIfSucceed() {

        runBlocking {
            useCase = GetCastCreditsByIdUseCase(repository)

            Mockito.`when`(repository.getCastCredits(anyLong())).thenReturn(
                Result.success(
                    expectedResult
                )
            )
